...
const MedicalDepartments = ({ data }) => {
    ...
    const rawDocs = data?.populatedData?.docs || data?.selectedDocs || [];

    const displayDept = rawDocs.map((item) => {
    ...
    // 3. Last Resort Fallback: If CMS actually has no items, use your static list
    const finalData = displayDept.length > 0 && displayDept[0].title !== undefined
        ? displayDept
        : "no dept";
        ...
        <section className={styles.section}>
            <div className={styles.container}>
                <div className={styles.header}>
                    <div className={styles.subTitle}>{renderContent(introContent, 'paragraph')}
                    ...
                    <div className={styles.heading}>
                        <h2 className={styles.title}>{renderContent(introContent, 'heading')}</h2>
                    ...
                    {displayDept.map((dept, index) => (
                        ...
                        >
                            <div className={styles.iconWrapper}>
                                <Image
                                    src={dept.icon?.url ? `${PAYLOAD_URL}${dept.icon.url}` : (dept.icon || null)}
                                    alt={dept.name || dept.title || 'department'}
                                    width={56}
                                    height={56}
                                    unoptimized={true}
                                    className={styles.img} />
                            </div>
                            <h3 className={styles.cardTitle}>{dept.name || dept.title}</h3>
