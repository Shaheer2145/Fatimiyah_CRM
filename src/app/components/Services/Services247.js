'use client';
import styles from './Services247.module.css';

import { motion } from 'framer-motion';

import Image from 'next/image';
import serviceImage2 from "../../assets/ServiceLayout.png";
...
const Services247 = ({ data }) => {
    if (data) {
        console.log("Service Page has got the connection", data);
    } else {
        console.error("Service Page not found", null);
    }
    ...
    const textGroup = data?.columns?.[0];
    // console.log("col1", textGroup);

    const mediaGroup = data?.columns?.[1];
    // console.log("col2", mediaGroup);


    const title = textGroup?.text;
    const paragraph = textGroup?.text;
    ...
    // console.log("Features Array:", data?.serviceSection?.[1]?.features);
            ...
            <div className={`container ${styles.content}`}>
                <div className={styles.leftContent}>
                    <div className={styles.header}>
                        <div className={styles.subHead}>{renderContent(paragraph, 'paragraph')}
                        ...
                        <h2 className={styles.heading}>{renderContent(title, 'heading')}</h2>
                    </div>
                    ...
                    >
                        {textGroup?.arraySection?.[0]?.features?.map((feature, index) => (

                            <motion.div className={styles.featureItem} variants={itemVariants} key={feature.id || index}>
                                <div className={styles.icon} >
                                    {feature.icon.url && (
                                        <Image
                                            src={`${PAYLOAD_URL}${feature.icon?.url}` || `${PAYLOAD_URL}${textGroup?.arraySection?.[0]?.features?.icon?.url}`}
                                            alt={feature.icon.alt}
                                            width={feature.icon.width}
                                            height={feature.icon.height}
                                            onError={(e) => {
                                                console.error("Failed loading Service Image ", e.message)
                                            }}
                                        />
                                    )}
                                </div>
                                <div className={styles.leftContentBox} >
                                    <h3 className={styles.serviceheading}>{feature.title}</h3>
                                    <p>
                ...
                <div className={styles.rightContent}>
                    <div className={styles.layoutEffectContainer}>
                        <Image
                            src={serviceImage2}
                            alt="service"
                            unoptimized={true}
                            className={styles.layoutEffect}
                            width={560}
                            height={600}
                        />
                    </div>
                    ...
                    <div
                        ...
                        style={{
                            backgroundImage: mediaGroup.media?.url ? `url(${PAYLOAD_URL}${mediaGroup.media?.url})` : `url(${PAYLOAD_URL}${mediaGroup.icon?.url})`
                        }}
                    >
                    </div>
                </div>
